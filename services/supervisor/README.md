...
本目录提供总控容器入口。入口只负责组合现有的固定四 Agent 架构，不加载 VLA、
YOLO 或 Isaac Sim 权重，也不会在缺少平台环境时退回 Mock。

## 强制启动条件

1. Agent 配置中的 VLA/YOLO SHA 必须替换为完整的
   `sha256:<64 位十六进制>`；
2. π0.5、OpenVLA-OFT 和 YOLO 的 URL 必须可访问，且 `/health` 内容与配置一致；
