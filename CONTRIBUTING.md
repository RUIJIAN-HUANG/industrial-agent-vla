...
2. 已冻结的系统架构不得由单个成员自行调整。确需变更时，必须先创建 Issue，说明官方依据、影响范围、回退方案，并由 A（项目负责人/总 Agent/集成）确认。
...
| 角色 | 主责 | 涉及其接口时必须请求其评审 |
|---|---|---|
| A | 项目管理、需求与评分冻结、总 Agent、状态机、路由、集成 | 总 Agent、跨模块契约、最终集成 |
| B | Isaac/Franka/夹爪、控制器、物理与仿真性能 | 仿真环境、执行器、安全状态机 |
