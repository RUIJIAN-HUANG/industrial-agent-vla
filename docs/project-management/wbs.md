...
| E30 总 Agent | TaskPlan/FSM、双 VLA 路由、核验、恢复与可观测性 | A | D01-D30 | G3/G5 | Partial（Mock） |
...
| E70 集成与实验 | 三任务闭环、恢复、路由、消融、OOD 与迁移 | A/F | D17-D35 | G3-G6 | No evidence |
...
| AGT-01 | 作为用户，我要自然语言被拆为有序语义子任务，而不是坐标脚本 | A | D17 | TaskSchema/TaskPlan/三模板/Schema | 无 pose/waypoint/grasp point；依赖无环；每项有可观测后置条件 |
| AGT-02 | 作为系统，我要每个动作后重新观察、核验并有界重决策 | A/B/F | D28 | FSM、单步滚动执行、Verifier、恢复 trace | 新 observation 后才下第二步；同策略重规划 <=1；切换 <=1；不回切 |
| AGT-03 | 作为工程团队，我要双执行器共享严格接口但保持独立环境 | A/D/E | D17 | health/infer/cancel schemas、适配器 | SHA/单位/ID/时长恶意响应被拒绝；两模型不共享 norm stats |
| VLA-D1 | 作为 OpenVLA owner，我要复现官方基线并返回 canonical N×7 | D/A/B | D14 | 固定 commit、环境、服务、100 次压测 | health SHA 正确；相机预处理 checksum；P50/P95/显存和错误码齐全 |
| VLA-D2 | 作为 OpenVLA owner，我要给出工业微调的可复现实证 | D/C/F | D21-D33 | RLDS/OFT、checkpoint、base/tuned 表 | 同 seed/同协议；训练配置/数据/权重 SHA；改进和回退均报告 |
| VLA-E1 | 作为 π0.5 owner，我要复现 openpi 并返回 canonical N×7 | E/A/B | D15 | 固定 commit/submodule、LeRobot/norm、服务 | 独立 JAX 环境；真实 norm SHA；cancel/超时/背压测试通过 |
| VLA-E2 | 作为 π0.5 owner，我要在公平冻结集上证明价值 | E/C/F | D22-D24 | checkpoint/离线或在线对照 | 有量化收益才双在线；否则转单主在线+离线对照 |
| VER-01 | 作为评委，我要看到开放域感知的 mAP 和速度 | F/C | D31 | ID/OOD 集、mAP、P50/P95、失败样例 | split 冻结；base/tuned 同协议；原始预测可重算 |
| VER-02 | 作为总 Agent，我要可靠区分成功、失败和不确定 | F/A | D26 | 3 帧 2 票、混淆矩阵、错误码 | 重复帧不重复投票；低置信不误判；GT 只做离线比对 |
| INT-01 | 作为团队，我要至少一个真实 VLA 完成首个闭环 | 全员/A | D18 | 20 局日志、视频、失败 Pareto | G3 通过；否则冻结第二模型并集中单主模型 |
| INT-02 | 作为队长，我要用公平数据决定双 VLA 拓扑 | A/D/E/F | D24 | 同 100 seed 对照、route table、ADR | 成功率/延迟/安全/区间齐全；执行 G4 去留决定 |
