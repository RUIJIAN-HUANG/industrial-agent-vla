...
1. 官方答疑对官方比赛方案的补充/澄清；
2. 官方比赛方案；
3. 当前冻结架构说明 `docs/architecture/agent-framework.md` 与接口契约；
...
| REQ-PLAN-001 | SRC-01 p2；SRC-02 p3 Q8 | 将高层指令拆为符合机器人作业逻辑的有序任务序列 | π0.5 的语言/动作序列能力 + 赛前冻结 TaskProfile/FSM；交接后仍由同一服务执行 Arm_B 搬箱阶段 | 任务阶段、依赖/后置条件、动作序列与推进/同角色恢复 trace | A/D/E |
...
| REQ-MA-001 | SRC-02 p1 Q3；p3 Q8 | 体现多智能体协同；官方示例为多个执行机构的并/串行分工 | 三 Agent 固定协作：Supervisor、YOLO、π0.5；双臂由同一 π0.5 按令牌串行进入共享区 | 完整事件链、交接令牌与双臂互斥 trace；一镜到底协作录像 | A/B/D/E/F |
...
| REQ-FT-001 | SRC-01 p3；SRC-02 p1 Q2 | 可调用现有大模型+prompt；工业场景微调是明确加分方向，必须说明改进效果 | 项目冻结为 π0.5 双臂工业微调；感知模型独立评测 | Arm_A/Arm_B 在同一 π0.5 服务上的 base/fine-tuned 对照、配置与 checkpoint SHA | D/E/F |
...
- 三 Agent 与双臂职责冻结：总控负责生命周期，YOLO 负责检测证据，π0.5 按
  `arm_id` 控制 Arm_A 或 Arm_B；Arm_A 完成装箱并把料箱放到
  `HANDOFF_CENTER` 后退回 `HOME_A`，Arm_B 只在 `B_ONLY` 阶段搬运
...
- π0.5 必须完成 Arm_A/Arm_B 各自观测流的工业场景微调和 base/tuned
...
|---|---:|---|---|---|
| SCORE-01 | 20 | 完整覆盖“感知-决策-执行” | 三 Agent 完整 trace、双臂交接视频、状态图、连续回合 | A/F |
...
| SCORE-04 | 10 | 任务序列合理性与执行成功率 | TaskProfile/FSM、π0.5 双臂动作序列、正式任务成功率、失败码分布 | A/D/E/F |
...
| DEL-02 | SRC-02 p3 Q9.2 | 完整训练代码、预训练权重来源、工业数据/生成式数据说明 | `services/pi05/`、`services/yolo/`、`data/` 清单 | 小样本训练 smoke |
