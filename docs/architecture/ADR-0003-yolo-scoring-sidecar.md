...
| OpenVLA-OFT Agent | 在 `handoff_ready` 已持久化且令牌为 `B_ONLY` 后，控制 Arm_B 把同一料箱搬到 `FINISHED_01` 并退出 | 控制 Arm_A、提前进入共享区 |
...
- 只有 `robot.arm_a.retreated=true` 且三张新鲜观测达到冻结票数后，才持久化
  `handoff_ready` 并授予 `B_ONLY`；
