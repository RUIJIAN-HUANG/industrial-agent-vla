...
当前状态（2026-09-05）：**V2 人工工业场景源码、训练、推理、Isaac Sim
GUI/物理/IK/抓取/满载搬运和真实闭环均已完成最终验收；完整 Python CI、模型
制品、证据索引和提交材料均已冻结并合入 `main`。**

## 最终提交状态

π0.5 工业策略训练、推理和最终验收均已完成，当前仓库为最终提交版：

- 任务合同、Supervisor、7D 动作安全边界、数据 Recorder/Reader、转换 Preflight
  和服务接口均已审计、验收并冻结；
- π0.5、YOLO checkpoint、norm stats、类别表和配置摘要均已完成制品绑定，生产
  部署由提交包注入完整 SHA-256；
- G0/G3/G5/DEL-04 以及 YOLO 证据均已复核，状态为 `REPRODUCIBLE`；
- 模型清单中的提交制品状态为 `FROZEN`，训练环境、评测报告、闭环结果和复现
  入口均已纳入最终提交材料。
